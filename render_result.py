...
for seq in seqs[:1]:
    if seq.endswith('.npy'):
        ...
        use_obj = True
        if use_obj:
            subject_template_path = "BIWI/templates/BIWI_topology.obj"
            ref_mesh = trimesh.load_mesh(subject_template_path, process=False)
        else:
            # Get template vertices from template_data dictionary
            if subject not in template_data:
                print(f"Warning: Subject {subject} not found in template_data. Skipping...")
                continue

            template_vertices = template_data[subject]
            # Create mesh using template vertices and topology faces
            ref_mesh = trimesh.Trimesh(vertices=template_vertices, faces=faces)
